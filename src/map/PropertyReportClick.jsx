...
class PropertyReportClick extends Component {
  ...
  onMapLoad() {
    ...
    if (urlARN !== null) {
      ...
      const parcelARNURL = parcelURLARNTemplate(window.appConfig.parcelLayer.url, urlARN);
    ...
    window.map.on("singleclick", async evt => {
      ...
      if (disable) return;
      const parcelURL = parcelURLTemplate(window.appConfig.parcelLayer.url, evt.coordinate[0], evt.coordinate[1]);
  ...
  onPropertyEmitter = coords => {
    const parcelURL = parcelURLTemplate(window.appConfig.parcelLayer.url, coords[0], coords[1]);
  ...
  getPopupContent = propInfo => {
    ...
    rows.push(
      <InfoRow className="sc-no-select" key={helpers.getUID()} label={"Tools"}>
        ...
        &nbsp;
        <span className="sc-fakeLink" onClick={() => helpers.showURLWindow(window.appConfig.termsUrl, false, "full")}>
          [Terms]
        </span>
      </InfoRow>
    );
      ...
      if (clickEvt === null) {
        ...
        helpers.getGeometryCenter(feature.getGeometry(), center => {
          ...
          if (feature !== undefined) {
            const infoURL = window.appConfig.propertyReportUrl + "?arn=" + arn;
