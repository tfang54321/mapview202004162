...
import "./SCMap.css";
import "./OLOverrides.css"; //BLI:override the OpenLayers icons on the map view area
import "./Navigation";
...
import * as olController from "../utilities/olController";
...
class SCMap extends Component {
  constructor(props) {
    ...
    this.loadingSymEnabled =this.loadingSymEnabled.bind(this);

    //add the map event listener
    window.emitter.addListener("tileloadstart",(source)=>{
      //show loading symbol when loading the map source data
      this.loadingSymEnabled(true);
    })
  }
  loadingSymEnabled(flag) {
    if(this.state.showLoadingSym !== flag) {
      this.setState({showLoadingSym:flag})
    }
  }
  componentDidMount() {
    ...
    var map = new Map({
      ...
      //disable the keyboard event in openlayer map level
      interactions: defaultInteractions({ keyboard: false, altShiftDragRotate: false,
         pinchRotate: false, mouseWheelZoom: false }).extend([
        new MouseWheelZoom({
          duration: 0,
    ...
    //add the event listener to handle the map viewport update
    map.on("postrender",()=>{
      //turn off the loading symbol after the map rended completely
      this.loadingSymEnabled(false);
    })
    if (storage !== null) {
    ...
    window.LayersInView=Util_SortedArray.getArray();
    ...
    window.map.getViewport().addEventListener("contextmenu", evt => {
      ...
      //BLI:disable the menu options by clicking right mouse button on the map view
      // const menu = (
      //   <Portal>
  ...
  getRenderContent =()=>{
    ...
    if(window.appConfig.BLI_Under_Dev.Dev_SimpleViewMap_only) {
      context=(
          ...
          <div className={this.state.showLoadingSym ? "" : "sc-hidden"}>
              <img className="mapView-main-loading" src={images["loading_transp.gif"]} 
               alt="loading" />
          </div>
        </div>
      );
